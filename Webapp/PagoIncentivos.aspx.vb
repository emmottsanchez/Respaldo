...
Public Class PagoIncentivos
    ...
    Private Sub LlenarDll()
        ...
        vNombreSP = "SET LANGUAGE Spanish SELECT  distinct(datename(year,Periodo)) as Año " & _
                    " FROM CPECPagoIncentivos where Rut=" & VarSQL(Session("UsuarioAct.Rut"), TipoVar.Numerico) & " and Year(Periodo)=2017"
        vArgumentos = ""
    ...
    Sub Nomb()
        vNombreSP = "select * from CPECUsuarios where Rut=" & VarSQL(Session("UsuarioAct.Rut"), TipoVar.Numerico) & _
                   " and Periodo=' " & ddlAño.SelectedValue & "-" & ddlMes.SelectedValue & "-01" & "'"
        vArgumentos = ""
        Dim ds As DataSet = cnn.EjecutarSP(vNombreSP, vArgumentos)
        If ds.Tables(0).Rows.Count > 0 Then
            lblNombPec.Text = " Usuario:  " & ds.Tables(0).Rows(0).Item("NomCorto")
        Else
            lblNombPec.Text = ""
        End If
